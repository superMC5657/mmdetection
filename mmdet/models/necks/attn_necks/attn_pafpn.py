...
from ...attn_module import FPNAttentionBottom, FPNAttentionBottomV2, FPNAttentionBottomV3
...
@NECKS.register_module()
class AttnPAFPNV2(AttnFPNV2, ABC):
    def __init__(self,
                 in_channels,
                 out_channels,
                 num_outs,
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            if i == self.backbone_end_level - 1:
                bottom_conv = False
            self.fpn_attn_bottom.append(
                FPNAttentionBottomV2(out_channels, downsample_cfg=downsample_cfg, bottom_conv=bottom_conv,
                                     self_conv=self_conv))
            self.pafpn_convs.append(ConvModule(
                out_channels,
                out_channels,
...
@NECKS.register_module()
class AttnPAFPNV3(AttnFPNV3, ABC):
    def __init__(self,
                 in_channels,
                 out_channels,
                 num_outs,
        ...
        for i in range(self.start_level, self.backbone_end_level - 1):
            self.fpn_attn_bottom.append(
                FPNAttentionBottomV3(out_channels, downsample_cfg=downsample_cfg))
            self.pafpn_convs.append(ConvModule(
                out_channels,
                out_channels,
                3,
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            laterals[i - 1] = (query_up + 1) * laterals[i]
        ...
        #   part 2: add bottom-up path
        for i in range(0, used_backbone_levels - 1):
            query_bottom, value_bottom = self.fpn_attn_bottom[i](inter_outs[i])
            query_self, value_self, key_self = self.fpn_attn_bottom[i + 1](inter_outs[i + 1])
            query = torch.cat((query_bottom, query_self), dim=-2)
            value = torch.cat((value_bottom, value_self), dim=-1)
            inter_outs[i + 1] = self.attention(query, value, key_self)
        outs = [self.pafpn_convs[i](inter_outs[i]) for i in range(self.start_level, self.backbone_end_level)]
